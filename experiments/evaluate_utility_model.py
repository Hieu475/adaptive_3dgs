...
Implements:
  - Evaluates baseline ladder B0 to B7 + Oracle strictly on independent cross_scene_test split (tum_fr2_xyz).
  - Computes RQ1 Prediction metrics: Spearman rho, Pearson r, MAE(Delta Q), MAE(Delta T), MAE(U), Calibration ECE/slope.
  - Multi-seed execution across protocol seeds [42, 43, 44, 45, 46] reporting Mean ± Std and 95% CI.
  - Evaluates geometry stratum breakdown (flat, edge, texture, depth_discontinuity).
  - Performs failure mode analysis (over-predicted U_hat >> U* vs under-predicted U_hat << U*).
  - Exports standardized interface records for Phase 5 (predictions_phase5.json).
  - Updates per-seed gate2.json and comprehensive benchmark reports.
"""
import os
import sys
import json
...
from research.utility_metrics import (
    ...
    analyze_utility_prediction_failures,
)
...
def main():
    ...
    # 1. Load canonical dataset using single source of truth
    dataset = UtilityDataset.from_oracle()
    train_ds, val_ds, test_ds, normalizer = prepare_normalized_splits(dataset=dataset)

    print(f">> Dataset Splits: Train={len(train_ds)} (fr1/desk 0-40), Val={len(val_ds)} (fr1/desk 41-60), Test={len(test_ds)} (fr2/xyz)")
    print(f">> Independent Test Scene: '{test_ds.metadata[0].scene}', N={len(test_ds)} samples")
    print(f">> Feature dimension: {len(test_ds.feature_names)} canonical features")
    ...
    per_seed_gate2 = {}
    phase5_records_by_seed = {}

    for seed in seeds:
        # B0: Random
        rnd_scorer = RandomScorer(seed=seed)
        rnd_scores = rnd_scorer.score(X_test_unnorm)
        m_rnd = evaluate_utility_complete(pred_u=rnd_scores, oracle_u=y_u_test, delta_q=y_q_test, costs=y_t_test)
        ...
        if os.path.exists(b5_path):
            ...
            m_b5 = evaluate_utility_complete(pred_u=pred_u_b5, oracle_u=y_u_test, delta_q=y_q_test, costs=y_t_test)
        ...
        if os.path.exists(b7_path):
            ...
            # Build standardized Phase 5 interface records for this seed
            pred_q_np = pred_q_b7.cpu().numpy()
            pred_t_np = pred_t_b7.cpu().numpy()
            pred_u_np = pred_u_b7.cpu().numpy()
            
            p5_list = []
            for idx, meta in enumerate(test_ds.metadata):
                p5_list.append({
                    "gaussian_id": meta.gaussian_id,
                    "predicted_delta_q": float(pred_q_np[idx]),
                    "predicted_delta_t": float(pred_t_np[idx]),
                    "predicted_utility": float(pred_u_np[idx]),
                    "oracle_utility": float(y_u_test[idx]),
                    "frame": meta.frame,
                    "scene": meta.scene,
                    "seed": seed,
                })
            phase5_records_by_seed[seed] = p5_list
    ...
    # Multi-seed methods (B0, B5, B6, B7)
    for name in ["B0: Random", "B5: Linear Utility", "B6: Two-Head Linear", "B7: Two-Head MLP (Ours)"]:
        runs = seed_runs[name]
        if len(runs) > 0:
            rhos = [r["spearman_rho"] for r in runs]
            ndcgs = [r["ndcg_20pct"] for r in runs]
            overlaps = [r["overlap_20pct"] for r in runs]
            regrets = [r["regret_20pct"] for r in runs]
            oses = [r["ose_20pct"] for r in runs]
            dqs = [r["realized_delta_q_20pct"] for r in runs]
            maes_u = [r["mae_utility"] for r in runs]
            eces = [r["calibration_ece"] for r in runs]

            vals_q = [r["mae_delta_q"] for r in runs if "mae_delta_q" in r and not np.isnan(r["mae_delta_q"])]
            vals_t = [r["mae_delta_t"] for r in runs if "mae_delta_t" in r and not np.isnan(r["mae_delta_t"])]

            aggregated_ladder.append({
                "method": name,
                "spearman_rho_mean": float(np.mean(rhos)),
                "spearman_rho_std": float(np.std(rhos)),
                "spearman_rho_ci95": compute_confidence_interval_95(float(np.std(rhos)), len(rhos)),
                "ndcg_20pct_mean": float(np.mean(ndcgs)),
                "ndcg_20pct_std": float(np.std(ndcgs)),
                "overlap_20pct_mean": float(np.mean(overlaps)),
                "overlap_20pct_std": float(np.std(overlaps)),
                "regret_20pct_mean": float(np.mean(regrets)),
                "regret_20pct_std": float(np.std(regrets)),
                "ose_20pct_mean": float(np.mean(oses)),
                "ose_20pct_std": float(np.std(oses)),
                "realized_delta_q_mean": float(np.mean(dqs)),
                "realized_delta_q_std": float(np.std(dqs)),
                "mae_delta_q": float(np.mean(vals_q)) if vals_q else float("nan"),
                "mae_delta_t": float(np.mean(vals_t)) if vals_t else float("nan"),
                "mae_utility": float(np.mean(maes_u)),
                "calibration_ece": float(np.mean(eces)),
            })
    ...
    print("\n" + "=" * 110)
    print(f"{'Method':<26} | {'Spearman ρ ↑':<18} | {'NDCG@20% ↑':<12} | {'OSE@20% ↑':<12} | {'Realized ΔQ':<15} | {'MAE(U) ↓':<10}")
    print("-" * 110)
    for row in aggregated_ladder:
        ...
        print(f"{m_name:<26} | {rho_str:<18} | {ndcg_str:<12} | {ose_str:<12} | {dq_str:<15} | {mae_str:<10}")
    print("=" * 110)

    # 7. Geometry Stratum Breakdown on Test Split (Seed 42)
    ...
    # 8. Failure Analysis (Requirement #20)
    print("\n>> Running Failure Analysis across geometric strata...")
    failure_report = analyze_utility_prediction_failures(
        pred_u=s_b7_np,
        oracle_u=y_u_test,
        X_features=X_test_unnorm,
    ...
    fail_dir = os.path.join(out_dir, "failure_analysis")
    p5_dir = os.path.join(out_dir, "phase5_interface")

    for d in [rq1_dir, baselines_dir, geom_dir, fail_dir, p5_dir]:
    ...
    # 9.2 Baselines and Geometry Artifacts
    ...
    # 9.3 Failure Analysis Artifacts
    with open(os.path.join(fail_dir, "failure_analysis.json"), "w") as f:
        json.dump(failure_report, f, indent=2)

    fail_md = os.path.join(fail_dir, "failure_analysis_report.md")
    lines_fail = [
        "# Phase 4: Systematic Failure Mode Analysis of Learned Utility Estimator",
        "",
        "## 1. Overview",
        f"- **Dataset Evaluated:** Independent cross-scene test set (`tum_fr2_xyz`, N={failure_report['n_samples']}).",
        f"- **Over-Predicted Gaussians ($\\hat U_i \\gg U_i^\\star$):** {failure_report['counts']['n_over_predicted']} samples (top {int(failure_report['threshold_quantile']*100)}% positive residual).",
        f"- **Under-Predicted Gaussians ($\\hat U_i \\ll U_i^\\star$):** {failure_report['counts']['n_under_predicted']} samples (bottom {int(failure_report['threshold_quantile']*100)}% negative residual).",
        "",
    ...
    for st, sdata in failure_report["strata_analysis"].items():
        lines_fail.append(
            f"| **{st.replace('_', ' ').title()}** | {sdata['total_samples']} | "
            f"{sdata['over_prediction_rate']:.1%} | {sdata['under_prediction_rate']:.1%} | "
            f"{sdata['mean_residual']:+.2e} | {sdata['mae_utility']:.2e} |"
        )
    lines_fail.extend([
        "",
    ...
    for d in failure_report["systematic_drivers"]["over_prediction_drivers"]:
        lines_fail.append(f"- **`{d['feature']}`**: z-shift = `{d['z_shift']:+.3f}` (elevated in over-predicted Gaussians)")
    lines_fail.append("\n### 3.2 Under-Prediction Drivers (Features with Largest Z-Shift in Under-Predicted Set)")
    for d in failure_report["systematic_drivers"]["under_prediction_drivers"]:
        lines_fail.append(f"- **`{d['feature']}`**: z-shift = `{d['z_shift']:+.3f}` (depressed in under-predicted Gaussians)")
    ...
    # 9.4 Standardized Phase 5 Interface Predictions
    p5_primary = phase5_records_by_seed.get(42, phase5_records_by_seed.get(seeds[0], []))
    with open(os.path.join(p5_dir, "predictions_test.json"), "w") as f:
        json.dump(p5_primary, f, indent=2)
    with open(os.path.join(out_dir, "predictions_phase5.json"), "w") as f:
        json.dump(p5_primary, f, indent=2)

    # 9.5 Comprehensive Benchmark Markdown Report
    ...
    lines = [
        ...
        f"- **Train Split:** `tum_fr1_desk`, frames 0–40 (N={len(train_ds)}).",
        f"- **Validation Split:** `tum_fr1_desk`, frames 41–60 (N={len(val_ds)}). Used strictly for model selection.",
        f"- **Independent Test Split:** `tum_fr2_xyz` (N={len(test_ds)}). Final zero-shot cross-scene evaluation.",
        f"- **Feature Normalization:** Mean and standard deviation fit strictly on train split only (zero leakage).",
    ...
    print(f"\n>> Saved RQ1 summary to: {rq1_dir}/summary.json")
    print(f">> Saved Baselines benchmark to: {baselines_dir}/benchmark.json")
    print(f">> Saved Geometry breakdown to: {geom_dir}/breakdown.json")
    print(f">> Saved Failure analysis to: {fail_dir}/failure_analysis.json")
    print(f">> Saved Phase 5 interface records to: {p5_dir}/predictions_test.json")
