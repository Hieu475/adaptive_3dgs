...
def main():
    ...
    parser.add_argument("--lambda-pt", type=float, default=0.25, help="Weight for pointwise loss.")
    ...
    print("=" * 80)
    print(f">> Seeds: {seeds}")
    print(f">> Epochs: {args.epochs} | LR: {args.lr} | Hidden: {args.hidden_dim} | Lambda_pt: {args.lambda_pt}\n")
    ...
    stats_file = os.path.join(stats_dir, "normalization.json")
    ...
    print(f">> Normalization saved to: {stats_file}\n")

    config = TrainingConfig(
        epochs=args.epochs,
        learning_rate=args.lr,
        lambda_pointwise=args.lambda_pt,
    )
