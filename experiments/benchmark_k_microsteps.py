...
def run_single_k_benchmark(
    ...
    """Execute pipeline for a given micro-step count K."""
...
def main():
    ...
    print(f"=== Multi-step Micro-convergence Benchmark: K in [1, 2, 3, 5] ===")
    ...
    for k in k_values:
        ...
        records.append(rec)
        print(f"   K={k} -> Mean ΔQ: {rec['mean_delta_q_db']:+.4f} dB | p50 opt: {rec['p50_opt_ms']:.2f} ms | Frame time: {rec['frame_time_ms']:.2f} ms | ΔQ/opt_ms: {rec['delta_q_per_opt_ms']:.4f}")
    ...
    manifest = create_provenance_manifest(
        ...
        scale_pixel_multiplier=1.5,
        initial_opacity=0.8,
        n_micro_steps=None,  # Variable K tested
    ...
    print("=" * 80)
    print(f"| {'K':<3} | {'Mean ΔQ (dB)':<12} | {'p50 opt (ms)':<12} | {'p95 opt (ms)':<12} | {'frame time (ms)':<15} | {'ΔQ / opt_ms':<12} |")
    print(f"|{'-'*5}|{'-'*14}|{'-'*14}|{'-'*14}|{'-'*17}|{'-'*14}|")
    for r in records:
        print(f"| {r['k']:<3} | {r['mean_delta_q_db']:<+12.4f} | {r['p50_opt_ms']:<12.2f} | {r['p95_opt_ms']:<12.2f} | {r['frame_time_ms']:<15.2f} | {r['delta_q_per_opt_ms']:<12.4f} |")
    print("=" * 80)
